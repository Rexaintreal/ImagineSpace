"""
generate_mesh.py  (combined patch: screenshots + pose auto-detect)
...
1. Screenshots use matplotlib (CPU-only) instead of Open3D's broken
   WSL offscreen GL renderer. You've already confirmed this works.
2. NEW: auto-detects whether `pose` is camera-to-world or
   world-to-camera by checking which gives a smaller/smoother
   frame-to-frame camera trajectory, and inverts if needed. This
   targets the comet-tail / stretched-wedge shape in your output.
   Prints its reasoning so you can see the numbers, not just trust it.
...
def detect_pose_convention(output_dir):
    """
    Check translation of consecutive frames under both 'pose as-is' and
    'pose inverted'. A camera moves a small, roughly-consistent distance
    between frames when the convention is correct; the wrong convention
    usually produces a much larger or far less consistent step size.
    Returns True if pose should be inverted before use.
    """
    camera_files = sorted(glob.glob(os.path.join(output_dir, "camera", "*.npz")))[:8]
    if len(camera_files) < 2:
        print("  Not enough frames to auto-detect pose convention, assuming as-is.")
        return False

    raw_positions, inv_positions = [], []
    for f in camera_files:
        pose = np.load(f)["pose"].astype(np.float64)
        raw_positions.append(pose[:3, 3])
        inv_positions.append(np.linalg.inv(pose)[:3, 3])

    def total_step(positions):
        positions = np.array(positions)
        steps = np.linalg.norm(np.diff(positions, axis=0), axis=1)
        return steps.mean(), steps.std()

    raw_mean, raw_std = total_step(raw_positions)
    inv_mean, inv_std = total_step(inv_positions)

    print(f"  As-is    pose: mean step {raw_mean:.4f}, std {raw_std:.4f}")
    print(f"  Inverted pose: mean step {inv_mean:.4f}, std {inv_std:.4f}")

    raw_score = raw_std / (raw_mean + 1e-9)
    inv_score = inv_std / (inv_mean + 1e-9)

    if inv_score < raw_score:
        print("  --> Detected: pose is WORLD-TO-CAMERA. Will invert before use.")
        return True
    else:
        print("  --> Detected: pose is CAMERA-TO-WORLD (as-is). No inversion needed.")
        return False


def backproject_frame(pose, intrinsics, depth, conf, color, conf_thresh, invert_pose):
    h, w = depth.shape
    ...
    pose_c2w = np.linalg.inv(pose) if invert_pose else pose
...
def merge_point_cloud(output_dir, conf_thresh, invert_pose):
    ...
    for f in camera_files:
        ...
        pts, cols = backproject_frame(pose, intrinsics, depth, conf, color, conf_thresh, invert_pose)
        if pts.shape[0] == 0:
...
def main():
    ...
    parser.add_argument("--force_invert_pose", action="store_true")
    parser.add_argument("--force_no_invert_pose", action="store_true")
    args = parser.parse_args()

    os.makedirs(os.path.dirname(args.out_pointcloud), exist_ok=True)

    if args.force_invert_pose:
        invert_pose = True
        print("Pose convention: forced INVERT (--force_invert_pose)")
    elif args.force_no_invert_pose:
        invert_pose = False
        print("Pose convention: forced AS-IS (--force_no_invert_pose)")
    else:
        print("Auto-detecting pose convention from first 8 frames...")
        invert_pose = detect_pose_convention(args.output_dir)

    print("Merging point cloud from all frames...")
    pcd = merge_point_cloud(args.output_dir, args.conf_thresh, invert_pose)
